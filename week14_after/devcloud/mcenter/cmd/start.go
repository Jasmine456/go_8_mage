...
var serviceCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := svr.start(); err != nil {
			if !strings.Contains(err.Error(), "auth: Server closed") {
				return err
			}
		}
...
func (s *service) start() error {
	...
	s.log.Infof("loaded auth app: %s", app.LoadedRESTfulApp())
	...
	for sg := range sign {
		...
		default:
			...
			if err := s.http.Stop(); err != nil {
				s.log.Errorf("auth graceful shutdown err: %s, force exit", err)
			} else {
				s.log.Infof("auth service stop complete")
			}
			return
		}
	}
