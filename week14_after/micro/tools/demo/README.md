...
```sh
$ 编辑样例配置文件 etc/demo.toml.book
$ mv etc/demo.toml.book etc/demo.toml
```
...
+ rpc： net/rpc
+ protobuf on tcp
+ gprc (protobuf on http2)
    + rpc service (gprc protobuf 通过service定义生成service接口, 默认实现了客户端的实例类(client sdk), Server端的实例类(not impl, 用于继承))
    + Registry: RegisterServiceServer (也是grpc框架帮忙生成的代码)的函数, 把实例类注册给Grpc服务器, 通过该实例类提供grpc的业务逻辑
    + server: main.go,  所有的实例类注册到grpc server 启动server 就启动了grpc服务
