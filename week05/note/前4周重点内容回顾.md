...
deferhoukeyigenyigefunc，func内部如果发生panic，会把panic暂时搁置，当把其他defer执行完之后再来执行这个。
defer后面不是跟func，而直接跟一条执行语句，则相关变量注册defer时被拷贝或计算。
```
...
panic 会顺序执行哪几步
	1. 倒序执行当前groutine的defer链(recover从这里介入)
	2. 打印错误信息和调用堆栈
	3.调用exit（2）结束整个进程

recover 会使程序从panic中恢复，并返回panic value。recover所在函数的后续的代码不会执行，但函数可以正常返回。在未发生panic是调用recover，会返回nil。recover()必须在defer中才能生效。
```
