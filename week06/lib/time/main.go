...
func basic() {
	...
	fmt.Printf("diff1=diff2 %t\n", int(diff1.Seconds()) == int(diff2.Seconds())) //在秒级上diff1和diff2是相等的
	fmt.Printf("t1>t0 %t\n", t1.After(t0))                                       //判断时间得先后顺序
...
func parse_format(){
	now := time.Now()
	ts := now.Format(TIME_FMT)//xxxx-xx-xx xx:xx:xx
	fmt.Printf("格式化时间%s %T\n",ts,ts) //Format返回值是string，需要使用Parse解析为时间格式

	t,_ := time.Parse(TIME_FMT,ts)// Format返回值是string，需要使用Parse解析为时间格式
	if t.Unix() != now.Unix(){
		fmt.Printf("Parse 时间解析错误：%d!=%d\n",t.Unix(),now.Unix() )
	}
	t,_ = time.ParseInLocation(TIME_FMT,ts,loc)
	if t.Unix() != now.Unix(){
		fmt.Printf("ParseInLocation 时间解析错误：%s\n",t.Format(TIME_FMT))
	}
	fmt.Println(t.Unix(),now.Unix(),t.Format(TIME_FMT))
...
func timer(){
	...
	//<-tm.C 尽量不连续使用，会死锁，但是测试未发现
	fmt.Printf("现在时间是%s\n",time.Now().Format(TIME_FMT))
	defer tm.Stop() //用完后记得stop
