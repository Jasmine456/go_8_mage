...
//基本数据类型
func variable() {
	fmt.Printf("os arch %s,int size %d\n", runtime.GOARCH, strconv.IntSize) // int是4字节还是8字节，取决于操作系统是32位还是64位的
	var a int = 5
	var b int8 = 5
	var c int16 = 5
	var d int32 = 5
	...
	var i uint32 = 5
	var j uint64 = 5
	fmt.Printf("a=%d,b=%d,c=%d,d=%d,e=%d,f=%d,g=%d,h=%d,g=%d,h=%d,i=%d,j=%d\n", a, b, c, d, e, f, g, h, i, j)
	var k float32 = 5
	var l float64 = 5
	fmt.Printf("k=%f,l=%.2f\n", k, l) // %.2f保留2位小数
	...
	fmt.Printf("m=%v,n=%v\n", m, n)                       // 按值的本来只输出
	fmt.Printf("m=%+v,n=%+v\n", m, n)                     // 在%v的基础上，对结构体字段名和值进行展开
	fmt.Printf("m=%#v,n=%#v\n", m, n)                     // 输出Go语言语法格式的值
	fmt.Printf("m的实部%f，m的虚部%f\n", real(m), imag(m))
	fmt.Printf("m的实部%e，m的虚部%g\n", real(m), imag(m)) // %e科学记数法，%g根据实际情况采用%e或%f格式（已获得跟简介，准确的输
	...
	fmt.Printf("q=%d,binary of q if %b\n", q, q)            //%b输出二进制
	var r rune = '☺'                                        //rune实际上是int32，既可以表示2147483647种字符，包括所有汉字和各种特殊字符
	fmt.Printf("r=%d,r=%U\n", r, r)                         //%U Unicode字符
	var s string = "I'm Jasmine"
	fmt.Printf("s=%s\n", s)
	var t error = errors.New("my error")
	fmt.Printf("error is %v\n", t)
	fmt.Printf("error is %+v\n", t) // 在%v基础上，对结构体字段名和值进行展开
	fmt.Printf("error is %#v\n", t) // 输出Go语言语法格式的值
