...
func switch_basic() {
	color := "yellow"
	// 当switch-case-default 模拟if-else
	switch color {
	...
	case "green": //case后面一个常量，只要switch 和case后面的值类型相同就行
		fmt.Println("go")
	case "red":
		fmt.Println("stop")
	case "yellow":
...
func switch_condition() {
	...
	case "green":
		fmt.Println("go")
	case "red", "yellow": //用逗号分割多个condition，他们之间是“或”的关系，只需要有一个condition满足就行
		fmt.Println("stop")
	}

	//	switch 后带表达式时，switch-case只能模拟相等的情况，如果switch后不带表达式，case后就可以跟任意的条件表达式
	switch {
	case add(5) > 10:
		fmt.Println("right")
	default:
...
func switch_type() {
	...
	switch num.(type) { //获取interface 的具体类型，.(type) 只能用在switch后面
	case int:
		fmt.Println("int")
	case float32:
		fmt.Println("float32")
	...
	case float64: // value  已被转换为float64类型
