...
func for_range() {
	//	数组、切片、string、map、channel都可以通过 for range的形式遍历
	arr := [5]int{1, 2, 3, 4, 5}
	for i, ele := range arr {
		fmt.Printf("%d: %d\n", i, ele)
	}
	...
	str := "我会唱ABC"
	for i, ele := range str {
		fmt.Printf("%d: %d %s\n", i, ele, string(ele)) //ele 是rune类型，而rune实际上是int32，所以要用%d
	}
	fmt.Println()
...
func range_copy() {
	arr := []int{1, 2, 3, 4, 5}
	for i, ele := range arr { // 如果arr里的元素是复杂结构体，建议使用结构体的指针，因为range 拷贝也是个不小的开销
