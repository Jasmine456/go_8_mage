...
func if_expression() {
	...
	if b := 8; b < 9 { // if 句子中允许包含1个(仅一个)分号，在分号前初始化一些局部变量(即只在if-else块内可见)
		fmt.Println("b<9")
	}
...
func else_expression() {
	...
	if color == "red" { // if只能有一个
		...
		fmt.Println("go")
	} else if color == "yellow" { // else if 可以有0个，一个或者连续多个
		fmt.Println("stop")
	} else { // else有0个或1个
	...
	if value, exists := m["tree"]; exists { // 初始化value和exists这两个局部变量，他们在if和else块里都是可见的
