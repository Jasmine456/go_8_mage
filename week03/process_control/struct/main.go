...
func init_struct() {
	...
	u = User{Id: 3, Name: "jasmine"} // 赋值初始化
	fmt.Printf("id=%d,enrollment=%v,name=%s\n", u.Id, u.enrollment, u.Name)
	u.enrollment = time.Now()                                               // 给结构体的成员变量赋值
	fmt.Printf("id=%d,enrollment=%v,name=%s\n", u.Id, u.enrollment, u.Name) //访问结构体的成员变量
	u = User{4, 100.0, time.Now(), "jasmine", "beijing"}                    // 赋值初始化，可以不写字段，但需要跟结构体定义里的字段顺序一致
...
func (_ User) think(man string) {
	fmt.Println("hi" + man + ", do you known my name? ")
}
...
func init_struct_ptr() {
	...
	user := &u    //通过取址符& 得到指针
	user = &User{ // 直接创建结构体指针
		Id: 3, Name: "jasmine", addr: "beijing",
	}
...
func anonymous_member() {
	type Student struct {
		Id      int
		string  //匿名字段
		float32 //直接使用数据类型欧威字段名，所以匿名字段中不能出现重复的数据类型
	}
	var stu = Student{Id: 1, string: "jasmine", float32: 79.5}
...
func nest_struct() {
	...
	type vedio struct {
		length int
		name   string
		user   //匿名字段，可以直接使用数据类型当字段名
	}
	v := new(vedio)
	v.length = 13
	v.name = "视屏名称"
	v.user.sex = 0       //通过字段名可以主机访问
...
//传slice，对slice的3个字段进行了拷贝，拷贝的是底层数组的指针，所以修改底层数组的元素会反映到原数组上
...
func main() {
	...
	fmt.Println("=======================")
	um := make(UserMap, 10)
	um[2] = User{Id: 2, Name: "jasmine", enrollment: time.Now()}
