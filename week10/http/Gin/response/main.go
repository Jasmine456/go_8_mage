...
func json1(engine *gin.Engine){
	engine.GET("/user/json1",func(c *gin.Context){
		c.JSON(http.StatusOK,gin.H{"name":"jasmine", "addr":"bj"}) //response Content-Type:aplication/json
	})
}
...
func jsonp(engine *gin.Engine) {
	...
	engine.GET("/user/jsonp",func(ctx *gin.Context){
		//	如果请求参数理由callback=xxx，则response Content-Type为applicaton/javascript,
		//	否则response Content-Type为application/json
...
func xml(engine *gin.Engine){
	...
	engine.GET("/user/xml",func(c *gin.Context){
		...
		//c.XML(http.StatusOK,gin.H{"name":"jasmine","addr":"sz"}) //respnse Content-Type:application/xml
	})
}
...
func html(engine *gin.Engine){
	...
	engine.GET("/user/html",func(c *gin.Context){
		// 通过json往前端页面上传值
		c.HTML(http.StatusOK,"template.html",gin.H{"title":"用户信息","name":"jasmine","addr":"sz"})
	})
}
