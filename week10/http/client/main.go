...
func simpleGet() {
	if resp, err := http.Get("http://localhost:5656"); err != nil {
		panic(err)
	} else {
		defer resp.Body.Close()

		fmt.Printf("response proto:%s\n", resp.Proto)
		fmt.Printf("respondse status:%s\n", resp.Status)
...
func simplePost() {
	...
	if resp, err := http.Post("http://localhost:5656", "text/plain", reader); err != nil { //Content-Type为text/plain,表示一个朴素的字符串
		panic(err)
	} else {
		defer resp.Body.Close() //注意：一定要调用resp.Body.Close()，否则会协程泄露（同时引发内存泄露）
		/*
			具体看一下http协议
		*/
		fmt.Printf("response proto:%s\n", resp.Proto)
...
// 通过form表单提交一些参数键值对
func postForm() {
	if resp, err := http.PostForm("http://127.0.0.1:5656", url.Values{"name": []string{"jasmine"}, "age": []string{"18"}}); err != nil {
		panic(err)
	} else {
		defer resp.Body.Close() //注意：一定要调用resp.Body.Close()，否则会协程泄露（同时引发内存泄露）
		/*
...
func head() {
	if resp, err := http.Head("http://localhost:5656"); err != nil {
		panic(err)
	} else {
		defer resp.Body.Close() //注意：一定要调用resp.Body.Close()，否则会协程泄露（同时引发内存泄露）
		fmt.Printf("response proto:%s\n", resp.Proto)
...
func restful() {
	if resp, err := http.Post("http://127.0.0.1:5656/user/xiaoming/vip/bj/haiding", "text/plain", nil); err != nil {
		panic(err)
	} else {
		defer resp.Body.Close()
	}
