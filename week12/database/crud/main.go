...
func insert(db *sql.DB){
	...
	lastId,err := res.LastInsertId() //ID自增，用过的ID（即使对应的行已delete）不会重复使用，如果使用单个INSERT语句将多行插入到表中，则LastInsert是第一条数据使用的id
	common.CheckError(err)
...
func replace(db *sql.DB){
	...
	common.CheckError(err)
	lastId,err := res.LastInsertId() //ID自增，用过的id（及时对应的行已delete）不会重复使用
	common.CheckError(err)
	fmt.Printf("after insert last id %d\n",lastId)
...
func query(db *sql.DB){
	...
	for rows.Next(){ //没有数据或发生error时返回false
		...
		err = rows.Scan(&id,&name,&city,&score) //通过scan把db里的数据赋值给go变量
		common.CheckError(err)
...
func main(){
	...
	*/
	...
	type Driver interface {
	    Open(name string,string) (Conn, error)
	}
	 */
