...
/*
自己实现一个函数链式的sql构建器。只是一个小demo，还需要完善
*/
...
func (self *OrderByBuilder) Ase() *OrderByBuilder{
	self.sb.WriteString(" asc ")
	return self
}
func (self *OrderByBuilder) Dese() *OrderByBuilder{
...
//And和Or都是where里的可选部分，他们的地位平等，都返回WhereBuilder
...
//通过select查询哪几列
func (self *SelectBuilder) Cloumn(columns string) *SelectBuilder{
...
func main(){
	//testDoubleLink()
	//Where,OrderBy,Limit 有没有都不影响调用ToString();Where里的And和Or有没有都不影响调用ToString()
	sql:=NewSelectBuilder("student").
		Cloumn("id,name,city").
		Where("id>0").And("city=''").Or("city=''").
		OrderBy("score").Dese().Limit(0,10).ToString()
