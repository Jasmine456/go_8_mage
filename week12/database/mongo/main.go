...
func update(ctx context.Context,collection *mongo.Collection){
	...
	update:=bson.D{{"$inc",bson.D{{"score",5}}}}// 符合条件 +5分
...
func query(ctx context.Context,collection *mongo.Collection){
	...
	filter:=bson.D{{"score",bson.D{{"$gt",3}}}}//score>3 ,gt代表 greater than
	findOption:=options.Find()
	findOption.SetSort(sort)
	findOption.SetLimit(10) // 最多返回10个
	findOption.SetSkip(3)//跳过前三个
...
func main(){
	...
	option:=options.Client().ApplyURI("mongodb://10.20.2.117:27017").
